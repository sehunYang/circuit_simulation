...
App.Analysis.tran=function(nl, devs, opts){
  ...
  /* t = 0⁺ */
  var r0=App.Analysis.newton(devs, N, assembleFor({mode:'tran',init:true,t:0,h:h,method:'BE'}), null, {maxIter:80});
  if(r0.err) return{err:r0.err};
  var x=r0.x;
  devs.forEach(function(d){ d.tranInit(x); });
  /* 0 번 표본: init 스탬프 기준 outputs */
  devs.forEach(function(d){ outs[d.id][0]=d.tranUpdate(x,{mode:'tran',init:true,t:0,h:h}); });
  t[0]=0; xs[0]=x;

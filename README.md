...
전원 전압(파랑), 저항 전압(청록), 저항 전류(주황)의 파형을 정상상태 마지막 세 주기만 보여 줍니다.
위 화면은 10 V 교류를 다이오드로 정류하고 100 µF 축전기로 평활한 것 — 봉우리에 매달려 톱니처럼 흔들리는
출력 전압이 교과서 그림 그대로 나옵니다. 소자를 클릭하면 그 소자의 v(t)·i(t) 로 바뀝니다.
...
> 방향 전환이 보일 만큼(60 Hz → 약 2.8 회/초) 시간을 늘려 재생합니다. 주파수가 높을수록 더 빨리 왕복하는 관계는 유지됩니다.

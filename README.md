...
| **GND · VCC** | 툴바 **⏚** 를 켜면 나타나는 접지·전원 레일 라벨 — 루프를 닫지 않고 회로이론 방식으로 그립니다 |
...
같은 이름의 VCC 라벨끼리는 도선 없이 이어진 것으로 계산합니다.
...
```bash
node tests/run-tests.js       # 엔진을 해석해와 대조 — 49개 시나리오 (260 검증)
node tests/browser/run.js     # 화면에 보이는 값·전자·POE·진리표 검증 — 헤드리스 Chrome (73 검증)
```

...
[![바로 실행](https://img.shields.io/badge/%EB%B0%94%EB%A1%9C%20%EC%8B%A4%ED%96%89-shy.ai.kr%2Fcircuit__simulation-2563eb?style=for-the-badge&logo=googlechrome&logoColor=white)](https://shy.ai.kr/circuit_simulation/)
[![GitHub Pages](https://img.shields.io/badge/GitHub%20Pages-%EB%B0%B0%ED%8F%AC%20%EC%A4%91-222?style=flat-square&logo=github)](https://shy.ai.kr/circuit_simulation/)
...
과도 응답 그래프와 오실로스코프, 시험지용 선화 내보내기, 학생이 먼저 예측하는 **POE 활동 28개**를 한 화면에 담았습니다.
...
**과도 응답 그래프**와 **오실로스코프**, 시험지에 그대로 붙일 수 있는 **선화 내보내기**,
그리고 학생이 먼저 예측하고 나서 확인하는 **예측·관찰·설명(POE) 활동**까지 한 화면에 있습니다.
저항·축전기·코일뿐 아니라 **전구·스위치·다이오드·트랜지스터(npn/pnp)** 를 다루고,
접지와 전원 레일로 그리는 **회로이론 표기**도 지원합니다.

### 👉 [**바로 실행하기**](https://shy.ai.kr/circuit_simulation/)

설치·회원가입 없이 브라우저에서 바로 열립니다. PC·태블릿·스마트폰 모두 지원합니다.
처음 열면 **시작 카드**가 떠서 예제 회로를 한 번에 만들어 보거나 POE 활동으로 넘어갈 수 있고,
막히면 **?** 버튼의 도움말(모드·표시 토글·단축키)과 화면 위 **상태 알림**(무엇이 잘못됐고 어떻게 고치는지)이 안내합니다.

![편집 화면](docs/images/01-edit.png)

| 🔌 소자 | 🎬 모드 | 📚 수업 |
|---|---|---|
| 전지·교류 전원, 저항·전구, 축전기·인덕터, **변압기(상호유도)**, 다이오드, npn·pnp 트랜지스터, 스위치·분기점, 접지·전원 레일 | 편집(값·전위 지형·전하 카운터) · 실행(전자 흐름·과도 그래프·오실로스코프) · 비유(수로·물레방아·플라이휠 3D) · 촬영(수능 규격 선화 PNG) | POE 예측·관찰·설명 28개 — 직류 기초, 축전기·인덕터, 여러 전원·전위, 교류, 반도체·논리(트랜지스터 AND·OR·NOT, 증폭·바이어스), 상호유도. 결과 CSV 진단 |
...
왼쪽 팔레트에서 소자를 **끌어다 놓으면** 격자에 붙습니다.
**포트가 맞닿으면 도선이 저절로 생기고**, 떨어져 있으면 포트(●)를 누른 채 끌어서 잇습니다.

| 팔레트 | 소자 |
|---|---|
| **DC · AC** | 전원 — 놓는 순간 **+ 단자 쪽에 스위치가 함께 생깁니다** (아래 참고). 속성에서 내부저항 r 도 줄 수 있습니다 |
| **R** | 클릭하면 **저항 / 전구** 를 고릅니다. 전구는 밝기가 전력 `I²R` 에 비례해 빛납니다 |
| **C · L** | 축전기 · 인덕터 |
| **L** | 클릭하면 **인덕터 / 변압기** 를 고릅니다. 변압기는 결합 인덕터 두 개 (상호유도) — 놓으면 1차·2차 코일이 나란히 생기고, 속성에서 **결합 계수 k** 를 바꿉니다. 인덕터 둘을 옆 칸에 두고 **옆 인덕터와 결합** 을 눌러도 됩니다. 2차 회로는 도선으로 이어져 있지 않아도 돌아갑니다 |
| **반도체** | 클릭하면 **다이오드 / npn / pnp** 를 고릅니다 |
| **J3 · J4** | 분기점 |
| **SW** | 추가 스위치 — 실행 모드에서 클릭해 켜고 끕니다 (논리 회로 입력) |
| **GND · VCC** | 툴바 **⏚** 를 켜면 나타나는 접지·전원 레일 라벨 — 루프를 닫지 않고 회로이론 방식으로 그립니다. **VCC 에 전위(예: 5 V)를 주면 그 자체가 접지 기준 전원**이 되어 전지 없이도 회로가 돌아갑니다 (0 V 면 같은 이름끼리 잇는 연결 라벨) |

> 🔀 **스위치가 자동으로 생기는 이유.** 편집 모드에서는 이 스위치가 **열려** 있고, **실행**·**비유** 모드로
> 들어가는 순간 **닫힙니다** — 그 순간이 `t = 0` 입니다. 그래서 편집 화면의 속성 패널은
> **"열림(지금) · 닫으면"** 두 값을 나란히 보여 줍니다. 열린 스위치 양단에 전지 전압이 그대로 걸려 있는 것을
> 편집 화면에서 늘 마주치게 됩니다. 편집 중에 닫힌 회로를 보고 싶으면 스위치를 클릭해 속성 패널의
> **"열림 → 닫기"** 를 누르세요 (**자동** 을 누르면 편집 열림 · 실행 닫힘 규칙으로 돌아갑니다).
> 실행 모드에서는 스위치를 직접 클릭해 여닫습니다.

![전위 지형과 열린 스위치](docs/images/14-potential-switch.png)

위 화면은 툴바의 **V(전위 지형)** 를 켠 모습입니다 — 도선이 전위에 따라 파랑(낮음)에서 빨강(높음)으로 칠해지고
전위 값이 적힙니다. 스위치가 열려 있으니 전지의 12 V 가 스위치 앞 한 토막에만 걸려 있고, 나머지는 전부 0 V 입니다.
...
실행 모드에서 할 수 있는 일이 더 있습니다.

| 조작 | 무엇을 보여 주나 |
|---|---|
| **스위치 클릭** | 켜고 끕니다. 병렬 가지를 추가하거나 논리 회로의 입력을 바꿀 때 |
| 툴바 **Q** | 전하 카운터 — 도선마다 지나간 전하량 `∫i dt` 가 올라갑니다. 전구 앞뒤 숫자가 같이 오르는 것이 "전류는 소모되지 않는다"의 증거 |
| 값을 바꾸면 | 배지가 3초 동안 **"이전 → 지금"** 을 함께 보입니다. 저항 하나를 바꿨는데 회로 전체가 바뀌는 것을 눈으로 |

축전기나 코일이 있으면 실행 모드는 **스위치를 닫는 순간부터의 과도 과정을 재생**합니다 —
RC 회로는 전자가 처음엔 많이 흐르다 충전이 끝나면 멎고, 과도 응답 그래프 위의 **세로 커서**가 지금 시각을 가리킵니다.
...
![반파 정류와 평활 축전기](docs/images/12-rectifier-scope.png)

교류 회로나 다이오드·트랜지스터가 있는 회로에서 실행 모드에 들어가면 **오실로스코프**가 열립니다.
그래프 아래 **소자 버튼**으로 어느 소자의 파형을 띄울지 켜고 끄고(소자마다 색, v(t) 는 실선·i(t) 는 점선),
`v(t)`·`i(t)` 버튼으로 전압·전류 전체를 끌 수 있습니다. 기본은 전원과 첫 저항(전구)이고, 캔버스에서 소자를 클릭하면 그 소자가 켜집니다.
창은 정상상태 마지막 두 주기이며, 실행 모드의 전자 재생이 같은 두 주기를 돌기 때문에 시간 커서가 창의 처음부터 끝까지 쓸고 갑니다.

> ⚡ **전류의 부호.** 소자는 저마다 자기 단자 순서를 기준으로 전류의 +를 정합니다. 그대로 그리면 같은 직렬 회로인데도
> 전원이나 돌려 놓은 소자의 파형이 180° 뒤집혀 보입니다. 그래서 오실로스코프는 **회로를 도는 방향**(전원에서 나가는 쪽)을
> + 로 잡아 소자마다 부호를 맞춥니다 — 직렬로 이은 소자의 전류 파형은 정확히 겹칩니다.
> 속성 패널의 값과 전력 부호는 소자 자신의 기준을 그대로 씁니다(전원의 공급전력이 +로 보이도록).

> 🔢 **교류 전원의 전압은 최댓값(진폭)입니다.** 220 V 로 두면 최댓값이 220 V, 실효값은 220/√2 ≈ 155.6 V 입니다.
> 가정용 220 V(실효값)를 재현하려면 최댓값을 311 V 로 두세요. 속성 패널에는 `전압(peak)` 과 `전압(RMS)` 이 함께 나옵니다.
위 화면은 10 V 교류를 다이오드로 정류하고 100 µF 축전기로 평활한 것 — 봉우리에 매달려 톱니처럼 흔들리는
출력 전압이 교과서 그림 그대로 나옵니다.

### 🔺 반도체 — 다이오드와 트랜지스터

![트랜지스터 스위칭](docs/images/13-transistor-switch.png)

**다이오드**는 순방향 약 0.6~0.7 V 에서 도통하고 역방향은 막습니다(Shockley 식). 속성 패널에 순방향 도통 / 역방향 차단 상태가 뜹니다.
**트랜지스터(npn·pnp)** 는 베이스·컬렉터·이미터 세 단자 전류와 V_BE·V_CE, 그리고 **동작 영역(차단 · 활성 · 포화)** 배지를 보여 줍니다.
위 화면은 4.7 kΩ 을 거친 2.4 mA 의 베이스 전류가 119 mA 의 컬렉터 전류를 열어 전구를 켜는 스위칭 작용 —
전지에서 나온 121 mA 가 2.4 + 118.8 로 갈라지는 것까지 배지에 보입니다.

이런 회로는 툴바 **⏚** 의 회로이론 표기(접지 GND · 레일 VCC)로 그리면 교과서 그림과 같아집니다.
같은 이름의 VCC 라벨끼리는 도선 없이 이어진 것으로 계산하고, VCC 라벨의 속성에서 **전위**를 주면
(예: `VCC 5 V`) 접지를 기준으로 하는 전원이 되어 전지 기호를 따로 그리지 않아도 됩니다.
음수(`VEE −5 V`)도 됩니다.
...
| **상호유도 (변압기)** | 두 플라이휠을 잇는 **굴대와 마찰판** | 판 사이가 떨어져 있어 회전의 **변화만** 건너갑니다 — 1차 바퀴가 가속·감속할 때만 2차가 돕니다 (교류 회로는 비유 모드가 없으므로 직류 변압기 예제에서만) |
| **분기점 (J3·J4)** | 갈림길 **수반** | 들어온 물과 나간 물이 같습니다 (= KCL) |
| **전구** | 물레방아 + **등불** | 물레방아가 발전기를 돌려 등을 켭니다. 밝기 = 순간 전력 ÷ 정격 |
| **스위치** | **수문** | 닫힌 스위치 = 문이 올라가 물이 지나감, 열린 스위치 = 문이 내려와 막음 |
| **다이오드** | **역류 방지 밸브** | 경첩 문이 순방향으로만 젖혀집니다. 역방향이면 닫힌 채 물이 서 있습니다 |
| **트랜지스터** | **제어 수문** | 베이스로 들어오는 작은 물길이 부표를 띄워 본류(컬렉터→이미터)의 큰 수문을 엽니다 — 차단·활성·포화에 따라 문 높이가 다릅니다 |
| **접지 (GND)** | **배수구** | 가장 낮은 곳. 물이 격자 배수구로 빠져나갑니다 |
| **레일 라벨 (VCC)** | **급수탑** | 같은 이름은 같은 탑. 전위를 준 라벨은 펌프가 물을 채워 올립니다 (= 전원) |
...
![POE 활동](docs/images/11-poe.png)

툴바의 **POE** 버튼을 누르면 오개념 진단 활동이 열립니다. 예제마다 회로를 불러오고,
학생이 **먼저 답을 고른 뒤**에 실행 모드로 들어가 확인하고, 마지막에 설명을 읽습니다.
오답 선택지 하나하나가 알려진 오개념 모델(전류 소모, 정전류 전지, 순차 추론, "열리면 전압도 0"…)이라
어떤 생각으로 틀렸는지 태그로 알려 줍니다.

| 예제 | 다루는 것 |
|---|---|
| 전구 밝기 순위 | 밝기 = 전력이지 전류가 아니다, 전류는 소모되지 않는다 |
| 병렬로 전구 추가 | 정전류 전지 모델, 병렬은 저항을 줄인다 |
| 열린 스위치 양단 전압 | 전압은 흐르는 것이 아니라 두 점 사이의 차 |
| 다이오드 정류 | 반파 정류 파형 (오실로스코프) |
| 트랜지스터 스위칭 | 작은 전류가 큰 전류를 제어 |
| 논리 회로 NOT · AND · OR | 진리표를 예측하고, 회로가 네 조합을 풀어 채워 준다 (AND·OR 는 교과서처럼 트랜지스터 직렬·병렬) |
| 축전기 충전 순간 · τ = RC · 직렬 축전기 전하 | 충전 전류는 처음이 최대, 시간 상수는 R 에도 비례, 직렬 축전기는 전하량이 같다 |
| 코일의 점등 지연 · LC 진동 · **스위치를 여는 순간** | 유도 기전력은 변화를 막는다, 코일과 축전기 사이를 오가는 에너지, 끊는 순간 120 V 가 튀어 전구가 번쩍인다 (실행 모드가 "여는 순간"부터 재생) |
| 직렬 두 전지의 전위 · 마주 보는 전지 · 전압이 다른 전지의 병렬 · **중첩** | 전위는 접지 기준 누적, 기전력의 방향, 낮은 전지는 충전된다, 전원 하나씩의 효과를 더한다 |
| 교류 실효값 · 축전기와 교류 · 코일과 교류 | 최댓값/√2, 충·방전 전류, 리액턴스 ωL 과 위상 — 주파수를 바꿔 가며 비교 |
| 트랜지스터 증폭 ①②③ · **β 와 포화** · **pnp** | 바이어스 없음(일그러짐) → 바이어스(온전한 증폭) → 과다(포화), 베이스 저항 1/10 씩, 베이스를 낮게 당겨야 켜지는 pnp |
| 상호유도 — 직류 전지와 변압기 | 2차 기전력은 `M·di/dt` — 전류가 **변할 때만**. 스위치를 닫는 순간 전구가 잠깐 켜진다 |
| 상호유도 — 교류 변압기의 전압비 | 전압비 = 권선비 `√(L₂/L₁)`, 전력은 보존 (전압이 2배면 전류는 절반) |

목록은 **분류 탭**(직류 기초 · 축전기·인덕터 · 여러 전원·전위 · 교류 · 반도체·논리 · 상호유도)으로 나뉘어 한 번에 한 분류만 보입니다.
몇몇 예제는 관찰 단계에 **비교 표**가 붙습니다 — 줄을 클릭하면 회로가 그 경우(주파수 10배, 결합 계수, 베이스 저항, 전지 하나를 0 V 로)로 바뀌고 측정값이 나란히 채워집니다.

결과는 **CSV 로 내보내** 학급 진단 자료로 쓸 수 있습니다.
...
| 툴바 **⏚ · V · Q** | 회로이론 표기 · 전위 지형 · 전하 카운터 |
| 툴바 **🔗** | 공유 링크 복사 — 회로가 URL 에 담깁니다. 붙여넣으면 그대로 열립니다 |
| 툴바 **POE** | 예측·관찰·설명 활동 |
...
| **인덕터 L** | 자속 `Φ = LI`, 자기에너지 `½LI²`. 결합된 인덕터면 짝·결합 계수 `k`·상호 인덕턴스 `M = k√(L₁L₂)` |
| **전원** | 공급전력 |
...
| **소비전력(평균)** (저항) | `I_rms² R` |
| **공급전력(평균)** (전원) | **유효전력** `V_rms I_rms cos φ` — 축전기·코일이 있으면 피상전력 `V_rms I_rms` 보다 작습니다 |
...
> 실행 모드의 전자는 **실제보다 느리게 왕복**합니다 — 60 Hz 를 그대로 보여 주면 떨림으로만 보이기 때문에
> 방향 전환이 보일 만큼(60 Hz → 약 1.4 회/초, 600 Hz → 2.5 회/초) 시간을 늘려 재생합니다. 주파수가 높을수록 더 빨리 왕복하는 관계는 유지됩니다.
> 전자의 **간격은 그 도선 전류의 진폭**을, **속력은 순시 전류(사인)** 를 나타냅니다 — 같은 전자가 제자리에서 한 칸쯤 왔다 갔다 하며,
> 전류가 0 을 지나는 순간 잠깐 멈춥니다.
...
| **전자기와 양자 (2022 개정)** | **p-n 접합 다이오드와 정류, 트랜지스터의 스위칭·증폭, 논리 회로(AND·OR·NOT)** |
...
| **반도체는 교과서 모델** | 다이오드는 Shockley 식(I_s = 1 pA), 트랜지스터는 Ebers-Moll 모델(β 는 속성에서)로 계산합니다. 실제 부품의 온도·주파수 특성은 없습니다 |
...
**Q. 만든 회로를 저장하거나 나눠 줄 수 있나요?**
툴바의 **🔗** 를 누르면 회로가 담긴 링크가 복사됩니다. 학생에게 그 링크를 주면 같은 회로가 열립니다.
그림으로는 **📷촬영**을 쓰세요.
...
```bash
node tests/run-tests.js       # 엔진을 해석해와 대조 — 51개 시나리오 (277 검증)
node tests/browser/run.js     # 화면에 보이는 값·전자·POE·진리표 검증 — 헤드리스 Chrome (94 검증)
```
